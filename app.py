# app.py — streamlined + rod-shaped cells (small, fast)
import json
import numpy as np
...
mode = st.sidebar.radio("Mode", ("Emission spectra", "Predicted spectra"))
...
def nls_unmix(Timg,E,iters=2000,tol=1e-6):
    H,W,C=Timg.shape; M=Timg.reshape(-1,C).astype(np.float32)
    C2,R=E.shape
    if C2!=C: raise ValueError(f"E shape {E.shape} mismatch with Timg channels {C}")
    scale=np.sqrt(np.mean(M**2,axis=1,keepdims=True)); scale[scale<=0]=1.0
    Mn=M/scale; EtE=E.T@E; A=Mn@np.linalg.pinv(EtE)@E.T
    A[A<0]=0
    for _ in range(iters):
        numer=Mn@E; denom=(A@(EtE))+1e-12
        A*=numer/denom
...
def _capsule_profile(H,W,cx,cy,length,width,theta):
    ...
    val=np.zeros((H,W))
    if np.any(rect): val[rect]=1-np.abs(yp[rect])/r
    for side in (-1,1):
        rho=np.sqrt((xp+side*half_L)**2+yp**2)
        cap=rho<=r
        if np.any(cap): val[cap]=np.maximum(val[cap],1-rho[cap]/r)
...
# -------------------- Simulation --------------------
def simulate_rods_and_unmix(E,H=160,W=160,rods_per=3,rng=None,noise_mode="per_channel"):
    rng=np.random.default_rng() if rng is None else rng
    C,R=E.shape
    ...
    peak=255
    lam=Tclean*(peak/np.max(Tclean))
    Tnoisy=rng.poisson(np.clip(lam,0,1e6))/peak
...
if use_pool:
    if not readout_pool:
        st.info("Readout pool not found."); st.stop()
    ...
    groups={p:[f for f in probe_map[p] if f in dye_db] for p in picked if probe_map.get(p)}
...
def run(groups,mode,laser_strategy,laser_list):
    ...
    if mode=="Emission spectra":
        ...
        if E_norm.shape[1]==0: st.stop()
        sel_idx,_=solve_lexicographic_k(E_norm,idx_groups,labels,levels=10,enforce_unique=True,required_count=required_count)
        ...
        for t,j in enumerate(sel_idx):
            y=E_norm[:,j]/(np.max(E_norm[:,j])+1e-12)
            fig.add_trace(go.Scatter(x=wl,y=y,mode="lines",name=labels[j],line=dict(color=_rgb01_to_plotly(colors[t]),width=2)))
        ...
        if not laser_list: st.error("Specify lasers."); st.stop()
        ...
        E_raw,E_norm,labels_all,idx_all=cached_build_effective_with_lasers(wl,dye_db,groups,laser_list,laser_strategy,powers_A)
        sel_idx,_=solve_lexicographic_k(E_norm,idx_all,labels_all,levels=10,enforce_unique=True,required_count=required_count)
        ...
        small_groups={"Pool":[l.split(" – ",1)[1] for l in final_labels]} if use_pool else {l.split(" – ",1)[0]:[l.split(" – ",1)[1]] for l in final_labels}
        E_raw_sel,E_norm_sel,labels_sel,_=cached_build_effective_with_lasers(wl,dye_db,small_groups,laser_list,laser_strategy,powers)
        colors=_ensure_colors(len(labels_sel))
        st.subheader("Selected Fluorophores (with lasers)")
        fluors=[l.split(" – ",1)[1] for l in labels_sel]
        _html_two_row_table("Slot","Fluorophore",[f"Slot {i+1}" for i in range(len(fluors))],fluors)
